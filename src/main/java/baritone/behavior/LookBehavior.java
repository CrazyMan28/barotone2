...
public final class LookBehavior extends Behavior implements ILookBehavior {
    ...
    private Rotation lastActualRotation;
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        ...
        switch (event.getState()) {
            case PRE: {
                ...
                this.lastActualRotation = actual;
            ...
            case POST: {
                ...
                if (this.prevRotation != null) {
                    Rotation visible = this.lastActualRotation == null ? this.target.rotation : this.lastActualRotation;
                    this.smoothYawBuffer.addLast(visible.getYaw());
                    while (this.smoothYawBuffer.size() > Baritone.settings().smoothLookTicks.value) {
                        this.smoothYawBuffer.removeFirst();
                    }
                    this.smoothPitchBuffer.addLast(visible.getPitch());
    ...
    private static abstract class AbstractAimProcessor implements ITickableAimProcessor {
        ...
        public final Rotation peekRotation(final Rotation rotation, boolean capVisibleBlockTurn) {
            ...
            Rotation wrapped = RotationUtils.wrapAnglesToRelative(prev, new Rotation(desiredYaw, desiredPitch));
            desiredYaw = wrapped.getYaw();
            desiredPitch = wrapped.getPitch();

            return new Rotation(
                    this.calculateMouseMove(prev.getYaw(), desiredYaw, Baritone.settings().undercoverLookYawSpeed.value, capVisibleBlockTurn),
                    this.calculateMouseMove(prev.getPitch(), desiredPitch, Baritone.settings().undercoverLookPitchSpeed.value, capVisibleBlockTurn)
            ).clamp();
        }
        ...
        private float calculateMouseMove(float current, float target, double undercoverMaxDegreesPerTick, boolean capVisibleBlockTurn) {
            float delta = target - current;
            if (capVisibleBlockTurn && Baritone.settings().strictVisibleBlockInteractions.value && undercoverMaxDegreesPerTick > 0) {
                float cap = (float) Math.max(0.25D, Math.min(90D, undercoverMaxDegreesPerTick));
                if (Math.abs(delta) > cap) {
                    target = current + Math.copySign(cap, delta);
