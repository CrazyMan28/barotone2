...
public final class Settings {
    ...
    /**
     * If true, block breaking / placing fallbacks wait for the visible crosshair to actually target the block
     * before clicking. This is slower and less permissive, but avoids looking one way while interacting another.
     */
    public final Setting<Boolean> strictVisibleBlockInteractions = new Setting<>(false);

    /**
     * Undercover profile movement aggression, from 0.0 (slow/cautious) to 1.0 (normal Baritone aggressiveness).
     */
    public final Setting<Double> undercoverMovementAggression = new Setting<>(0.22D);

    /**
     * Maximum visible yaw turn speed, in degrees per tick, while strict visible interactions are enabled.
     * Set to 0 to disable the cap.
     */
    public final Setting<Double> undercoverLookYawSpeed = new Setting<>(3.5D);

    /**
     * Maximum visible pitch turn speed, in degrees per tick, while strict visible interactions are enabled.
     * Set to 0 to disable the cap.
     */
    public final Setting<Double> undercoverLookPitchSpeed = new Setting<>(2.5D);

    /**
     * Smooth look averaging window applied by the undercover profile.
     */
    public final Setting<Integer> undercoverSmoothLookTicks = new Setting<>(22);

    /**
     * Right-click delay applied by the undercover profile.
     */
    public final Setting<Integer> undercoverRightClickDelay = new Setting<>(16);

    /**
     * Block-break delay applied by the undercover profile.
     */
    public final Setting<Integer> undercoverBlockBreakDelay = new Setting<>(18);
    ...
    /**
     * AI provider used by the {@code ai} command. Supported values: {@code mistral}, {@code ollama}.
     */
    public final Setting<String> aiProvider = new Setting<>("mistral");

    /**
     * Mistral AI API key used by the {@code ai} command. Stored locally in your Baritone settings.
     * Keep this private. Use {@code mistral key <key>} to set it without typing it in chat history.
     */
    public final Setting<String> mistralApiKey = new Setting<>("");

    /**
     * Mistral model name used by the {@code ai} command. Examples:
     * {@code mistral-large-latest}, {@code mistral-medium-latest}, {@code open-mistral-nemo}.
     */
    public final Setting<String> mistralModel = new Setting<>("mistral-large-latest");

    /**
     * Mistral chat completions endpoint. Override to point at a proxy or compatible service.
     */
    public final Setting<String> mistralEndpoint = new Setting<>("https://api.mistral.ai/v1/chat/completions");

    /**
     * Maximum tool-call rounds per {@code ai} goal. {@code 0} or less means unlimited until {@code done},
     * {@code ai stop}, or an API error.
     */
    public final Setting<Integer> mistralMaxIterations = new Setting<>(0);

    /**
     * Maximum response tokens the Mistral model may emit per turn.
     */
    public final Setting<Integer> mistralMaxTokens = new Setting<>(1024);

    /**
     * Sampling temperature for Mistral chat completions (0.0 - 1.5).
     */
    public final Setting<Double> mistralTemperature = new Setting<>(0.2);

    /**
     * If true, the AI agent will log every tool call and tool result to chat. Useful for debugging
     * and for understanding what the model is doing. Set to false for quieter operation.
     */
    ...
    /**
     * Ollama model name used by the {@code ai} command when {@link #aiProvider} is {@code ollama}.
     */
