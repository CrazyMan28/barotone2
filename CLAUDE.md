...
- **`ReflexProcess` (in `baritone/process/`) + `ReflexPlanner`** — every-tick survival guardian (escape lava > anti-drown > flee creeper > fight back > eat) that runs as a high-priority temporary Baritone process so any interrupted mine/goto resumes automatically. `ReflexPlanner` is the pure, unit-testable decision core.
